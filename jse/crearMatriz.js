...
function crear(){
    ...
    for(i=0; i<filas; i++){
        if (i!=filas-1) {
            HTML += "<td align=center>"+'<input type="text" id="vr'+i+'"  onkeypress="return valideKey(event, this);" style="width: 30px;" >,</td>';
        }else{
            HTML += "<td align=center>"+'<input type="text" id="vr'+i+'" onkeypress="return valideKey(event, this);" style="width: 30px;" ></td>';
        }
    }
...
function calcular(){
    ...
    for(i=0; i<filas; i++){
        for(j=0;j<n_col;j++){
            ...
            if(j!=columnas){
                if(document.getElementById("vm"+i+"_"+j).value=="" || document.getElementById("vm"+i+"_"+j).value=="-"){
                    contador++;
                }else if(document.getElementById("vm"+i+"_"+j).value==0){
                    contador0++;
                }
                
            }else{
                if(document.getElementById("vr"+i).value=="" || document.getElementById("vr"+i).value=="-"){
                    contador++;
                }else if(document.getElementById("vr"+i).value==0){
                    contador0++;
                }
                //matriz[i][j] = 'x';
            }
    ...
    for (j = 0;  j< columnas; j++) {
        for (i = 0; i < filas; i++) {
            if(document.getElementById("vm"+i+"_"+j).value==0){
                contador02++;
            }
            
        }
        if(contador02==filas){
            alert("Pon por lo menos un número diferente de 0 en los vectores muestra.");
            return false;
        }
        contador02=0;
    ...
    if(esBase){
        document.getElementById('combinacionLineal').insertAdjacentHTML('beforeend', "<br><br>&there4;El conjunto de vectores son una base.");
    }else{
        document.getElementById('combinacionLineal').insertAdjacentHTML('beforeend', "<br><br>&there4;El conjunto de vectores no son una base.");
    }
